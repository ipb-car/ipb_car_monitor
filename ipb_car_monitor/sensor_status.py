...
class SensorStatus:
    ...
    def __init__(self, node):
        ...
        with open("config/config_to_monitor.yaml", "r") as file:
            self.monitored_topics = yaml.safe_load(file)
        self.ready_to_record_pub = node.create_publisher(
            Bool, "ready_to_record", qos_profile=1
        )
        self._binary_subs = {}
    ...
    def listen_topics(self):
        ...
        for topic_name, _ in new_topics:
            ...
            msg_types = ["Image", "Imu", "NavSatFix", "PointCloud2", "TimeReference"]
            for msg_string in msg_types:
                msg_type = getattr(import_module("sensor_msgs.msg"), msg_string)
                try:
    ...
    def check_ready_to_record(self):
        ...
        if is_rpi_synchronized():
            boolean = Bool
            boolean.data = True
            self.ready_to_record_pub.publish(boolean)
    ...
    def check_consistency(self):
        ...
        np_diff_bool = self.np_sens_diff < self.PTP_OFFSET_LIMIT
