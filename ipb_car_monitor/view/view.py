...
class View:
    ...
    def __init__(self, controller, model):
        ...
        # Create the key press event thread.
        self.thread_key = threading.Thread(target=self.key_press_event)
        self.thread_key.start()

    def key_press_event(self):
        """Key press event thread.
        When the letter q is pressed it notify the controller to shutdown the program.
        """
        ...
        while self.controller.running_status():
            ...
            if self.cur_key == ord("q"):
                self.controller.action_shutdown()
            elif self.cur_key == 3:
                self.controller.action_shutdown()
    ...
    def update_windows(self):
        ...
        # Show the menu
        self.display_status(
            self.model.get_ptp_sensors(),
            self.win_sensors,
            self.dim_win_ptp,
            "Sensor status",
        )
        self.display_status(
            self.model.get_cameras(),
            self.win_cameras,
            self.dim_win_cameras,
            "Camera consistency",
        )
        self.display_logs(
            self.model.get_ptp_logs(),
            self.win_status_sensors,
            self.dim_win_ptp_logs,
            "PTP logs",
        )
        self.display_logs(
            self.model.get_cameras_logs(),
    ...
    def display_status_bar(self, win):
        ...
        Display key commans, windows size and last key pressed.
        """
        # win.clear()
        win.bkgd(curses.color_pair(2))
        win.box()
        ...
        win.addstr(1, 29, "Key: {}".format(self.cur_key))
        win.refresh()

    def quit(self):
        """Quit method invoked when program as to be shutted down."""

        # Generate key press 'q' in order to
        # exit from the key_press_event thread.
        press("q")
